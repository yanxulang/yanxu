...
`标准:`命名空间目前提供 17 个模块：1.0 的`文字`、`数学`、`时间`、`文件`、`JSON`、`网络`、`测试`、`路径`、`环境`、`哈希`、`编码`、`统计`与`CSV`，以及发行后新增的`随机`、`标识`、`模板`与`校验`。这些模块在树解释器和独立 VM 中使用同一套公开 API；可移植算法由两端共享实现。
...
完整示例见[`examples/扩展标准库.yx`](examples/扩展标准库.yx)与[`examples/后1.0标准库.yx`](examples/后1.0标准库.yx)。
