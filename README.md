...
`标准:`命名空间在 1.1.2 开发线提供 22 个模块：1.0 的`文字`、`数学`、`时间`、`文件`、`JSON`、`网络`、`测试`、`路径`、`环境`、`哈希`、`编码`、`统计`、`CSV`、`随机`、`标识`、`模板`与`校验`，1.1 新增的`Base64`、`正则`、`URL`和`日期`，以及 1.1.2 的`套接字`。这些模块在树解释器和独立 VM 中使用同一套公开 API；可移植算法由两端共享实现。
...
1.1.2 的`套接字`模块提供 TCP/UDP UTF-8 文字收发。连接、接受、发送和接收都必须显式给出毫秒超时；接收还必须给出不超过 4 MiB 的字节上限。模块复用`network`权限白名单，错误使用稳定`SOCKET_*`代码；完整入口见[`examples/1.1套接字.yx`](examples/1.1套接字.yx)。
...
完整示例见[`examples/对象与类型判断.yx`](examples/对象与类型判断.yx)、[`examples/扩展标准库.yx`](examples/扩展标准库.yx)、[`examples/后1.0标准库.yx`](examples/后1.0标准库.yx)、[`examples/1.1标准库.yx`](examples/1.1标准库.yx)与[`examples/1.1套接字.yx`](examples/1.1套接字.yx)。
