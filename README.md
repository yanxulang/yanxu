...
`标准:`命名空间在 1.1.3 稳定版提供 22 个模块：1.0 的`文字`、`数学`、`时间`、`文件`、`JSON`、`网络`、`测试`、`路径`、`环境`、`哈希`、`编码`、`统计`、`CSV`、`随机`、`标识`、`模板`与`校验`，1.1 新增的`Base64`、`正则`、`URL`和`日期`，以及 1.1.2 的`套接字`。这些模块在树解释器和独立 VM 中使用同一套公开 API；可移植算法由两端共享实现。
