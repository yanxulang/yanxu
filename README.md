...
核心 CLI 保留`yanxu 包`、`包 运行`、`包 锁`和`包 更新`作为运行时兼容入口；创建项目、增删依赖、安装与日常包管理请使用独立的官方工具[言包（yanbao）](https://github.com/YanXuLang/yanbao)。言包直接复用本仓库的`yanxu::package`解析器，因此`言序.toml`和`言序.lock`只有一套格式与解析语义。Git 与索引包的源码根在通过锁文件校验后自动进入模块加载边界，不需要把全局缓存路径写入应用文件权限；依赖清单声明的其他宿主权限不会自动传递。
...
`标准:`命名空间在 1.1.4 稳定版提供 23 个模块：1.0 的`文字`、`数学`、`时间`、`文件`、`JSON`、`网络`、`测试`、`路径`、`环境`、`哈希`、`编码`、`统计`、`CSV`、`随机`、`标识`、`模板`与`校验`，1.1 新增的`Base64`、`正则`、`URL`和`日期`，1.1.2 的`套接字`，以及 1.1.4 的`字节`。这些模块在树解释器和独立 VM 中使用同一套公开 API；可移植算法由两端共享实现。
