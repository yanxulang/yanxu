...
这是言序编程语言的 Rust 实现仓库。1.1.5 包含前端诊断、静态检查、树解释器、字节码 VM、完整包图、YXB 应用编译、原生扩展 ABI、嵌入 API、REPL 和完整工具链。
...
核心 CLI 保留`yanxu 包`兼容入口；创建项目、增删依赖、安装与日常构建请使用官方工具[言包（yanbao）](https://github.com/YanXuLang/yanbao)。言包 0.2.0 完全由言序源码实现，因此先安装言序 1.1.5 即可直接运行，不需要 Rust 或单独编译言包。它通过版本化 JSON 工程协议调用`yanxu-package`，不复制 TOML、版本选择、锁文件或 YXB 语义。
...
原生扩展 ABI v1 使用 C 兼容布局和 UTF-8 JSON 传值，支持函数、常量、不透明宿主资源、结构化错误、释放回调与言序回调入口。加载前必须同时通过目标平台、SHA-256、ABI 版本和`原生扩展`权限门禁；WASI 始终禁止动态库。参见[原生 ABI v1](reference/native-abi-v1.md)和[1.1.5 迁移指南](reference/migration-1.1.5.md)。
