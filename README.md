...
言序是一门面向现代软件工程的中文编程语言。它使用中文关键字、标识符与诊断信息，同时提供静态检查、模块与包、结构化错误、字节码虚拟机、自包含应用和可嵌入运行时。1.1.8 延续 1.1.7 的原生 ABI v2、官方桌面 GUI 包和跨平台 Bundle，并修复 Windows 图形回调路径的运行时栈不足。
