...
装载门禁。磁盘包只允许当前模块所属的锁定包加载自身制品，或从它的直接锁定依赖边
按真实包名选择原生模块；选择复用只读解析缓存以及源码根/generation 归属，不在运行时
改写锁文件。解析器返回的 generation 根和已经打开的制品令牌会一直消费到摘要校验，
不会在解析后按环境路径重新打开。
YXB 格式 1 继续消费归档内已经验证的字节；该格式没有保存逐包依赖边，因此不把扁平
原生模块映射描述为磁盘直接边授权。

装载前还会静态解析宿主格式和动态依赖，三平台均只接受当前架构的 64 位小端映像：
Linux 要求带 `PT_DYNAMIC` 的 ELF 共享库，拒绝 `DT_RPATH`、`DT_RUNPATH`、audit/filter
装载标签以及含路径、动态令牌或非常规字符的 `DT_NEEDED`；macOS 只接受 Mach-O 动态库
或 bundle，拒绝 `LC_RPATH`、环境/动态链接器/旧式依赖命令、未知装载命令及非
`/usr/lib`或`/System/Library`固定路径依赖；Windows 只接受 PE32+ DLL，拒绝延迟导入
和非常规导入名。依赖项数量、原始导入描述符数量和名称长度均有硬上限。
