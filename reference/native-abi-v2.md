...
权限查询使用 UTF-8 能力名。任何原生包都要求顶层应用显式开启`原生扩展`，包括官方
`yanxu-gui`。`图形界面`只授予 GUI 能力，不授予动态库装载权限；剪贴板、文件对话框、
通知、托盘、外部地址和全局快捷键仍分别授权，不能由 GUI 权限推导。

## 制品门禁

ABI v2 复用 v1 的普通文件、目标三元组、声明大小、SHA-256、私有内容寻址暂存和只读
装载门禁。磁盘包只可从当前包的直接锁定依赖边选择原生模块；解析器返回的 generation
根和已经打开的制品令牌会一直消费到摘要校验，不会在解析后按环境路径重新打开。
YXB 继续消费归档内已经验证的字节。

装载前还会静态解析宿主格式和动态依赖：Linux 只接受当前架构的 ELF 共享库，拒绝
`DT_RPATH`、`DT_RUNPATH`、audit/filter 装载标签和含路径的 `DT_NEEDED`；macOS 只接受
当前架构的 Mach-O 动态库或 bundle，拒绝 `LC_RPATH`、`LC_DYLD_ENVIRONMENT`及非
`/usr/lib`或`/System/Library`固定路径依赖；Windows 只接受当前架构的 PE DLL，拒绝
延迟导入和含路径的导入名。依赖项数量和名称长度均有硬上限。

Unix 以 `RTLD_NOW | RTLD_LOCAL`一次解析全部符号；Windows 使用
`LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR | LOAD_LIBRARY_SEARCH_SYSTEM32`，应用目录、当前目录
和用户追加 DLL 目录都不参与搜索。需要随包携带未声明私有动态依赖的旧制品必须改为
静态链接或系统依赖后重新构建；当前包格式不会把未锁定的相邻动态库视为可信闭包。
WASI 始终禁止动态库。原生模块仍是进程内受信任代码，门禁不能隔离恶意系统调用。
